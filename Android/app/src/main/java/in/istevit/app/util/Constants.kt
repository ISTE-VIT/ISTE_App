package `in`.istevit.app.util

class Constants {
    companion object{
        const val ISTE_SHARED_PREFERENCES = "ISTE_SHARED_PREFERENCES"
        const val INSTAGRAM_URL = "https://www.instagram.com/iste_vit_vellore/?hl=en"
        const val FACEBOOK_APP_URL = "fb://facewebmodal/f?href=https://www.facebook.com/ISTE.VIT/"
        const val FACEBOOK_URL = "https://www.facebook.com/ISTE.VIT/"
        const val GITHUB_URL = "https://github.com/ISTE-VIT"
        const val LINKEDIN_URL = "https://in.linkedin.com/company/indian-society-for-technical-education"
        const val YOUTUBE_URL = "https://www.youtube.com/c/istevitvellore"
        const val ISTE_WEBSITE = "https://istevit.in/"
    }
}